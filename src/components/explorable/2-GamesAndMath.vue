<template>
  <ScrollytellSection :num-groups="7" @slideChange="slideChange">
    ...
    <template v-slot:sticky="{current, progress, enterProgress, direction, leaveProgress}" v-if="!isMobile">
      ...
      <GamePlayer :segments="racket.segments" :startingPlayer="'blue'" :showTurn="false"
                  :style="Object.assign({opacity: 1-leaveProgress}, translateUp(Math.min((-429 + (current + progress) * 20), -2400 + (current + progress + enterProgress) * 350, 3000)))" class="absolute ml-16 w-2/3"/>
    </template>
    <template v-slot:absolute="{current, progress, enterProgress}" v-if="!isMobile">

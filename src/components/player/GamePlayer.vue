<template>
  <div class="w-full h-full">
    ...
    <g>
      <Scissors
        v-for="color in [Color.Red, Color.Blue]"
        ...
        :style="{
          transform: scissorsRenders[color].transform,
    ...
    <svg ref="svg" class="w-full h-full" viewBox="0 0 100 100">
      ...
      <template
        v-for="[id, { segment, style, animating, cut }] in Object.entries(
          segmentRenders
      ...
      >
        <g class="absolute" :style="style" v-if="animating || !cut">
          ...
          <PiecePath
            :segment="segment"
            :class="{ clickable: clickable(segment) }"
...
const props = defineProps({
  ...
  promptReset: {
    type: [Object, Boolean] as PropType<
      { text: string; choosePlayer?: boolean; subgraph?: string } | false
    >,
    default: () => ({
      text: "Play Again",
      choosePlayer: true,
...
const hasGreen = computed(() =>
  Object.values(graph.value.getLiveSegments()).some(
    (segment) => segment.color == Color.Green
  )
);
...
Object.values(scissorsRenders).forEach(
  (scissors) =>
    (scissors.transform = computed(() => {
      const { translateX, translateY, rotation } = scissors;
      return `translateX(${translateX}px) translateY(${
        translateY + props.scissorsOffsetY
    ...
    Object.values(graph.value.getLiveSegments()).map((segment) => {
      const obj = reactive({
        ...
        style: computed(() => {
          ...
          const opacity =
            typeof overrideOpacity == "number" && overrideOpacity < 1
              ? overrideOpacity
              : obj.opacity;
          return {
...
function resetGame(
  ...
  doResetScissors = true,
  startingPlayer = props.startingPlayer,
  subgraph = props.subgraph
) {
  if (props.debugMode) {
...
function playerSelected(params: { aiControls: Player[]; starting: Player }) {
  ...
  resetGame(
    ...
    true,
    params.starting,
    props.promptReset.subgraph || props.subgraph
  );
}
...
function animateScissors(
  ...
  const svgCutpoint = el?.getPointAtLength(
    (0.3 + Math.random() / 2) * el.getTotalLength()
  );
  const cutpoint = (dim: "x" | "y") =>
    svgCutpoint?.[dim] || (segment.start[dim] + segment.end[dim]) / 2;
  ...
  const duration =
    Math.sqrt((lastPos.x - x) ** 2 + (lastPos.y - y) ** 2) / speed;
  lastPos.x = x;
  lastPos.y = y;
